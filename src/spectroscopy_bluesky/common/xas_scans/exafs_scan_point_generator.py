...
class XasScanPointGenerator:
    ...
    def calculate_energy_time_grid(self) -> NDArray:
        ...
        if not self.is_constant_exafs_time():
            exafs_times = self.create_varying_time_exafs(
                exafs_energies, self.params.exafsFromTime, self.params.exafsToTime
            )

        # Smooth out the first few energy points in exafs region
        if self.smooth_exafs_region:
            num_energies_to_replace, smoothed_exafs = self.create_smoothed_exafs(
                self.params.c, self.params.edgeStep, exafs_energies
            )
        ...
        return np.concatenate(
            (pre_edge_region, ab_region, bc_region, exafs_region), axis=0
        )
    ...
    def create_pre_edge(self):
        points = self.create_const_step_region(
            self.params.initialEnergy, self.params.a, self.params.preEdgeStep
        )
    ...
    def create_AB_region(self):
        points = self.calculateVariableStepRegion(
            self.params.a, self.params.b, self.params.preEdgeStep, self.params.edgeStep
        )
        return self.create_energy_time_array(points, self.params.preEdgeTime)

    def create_BC_region(self):
        points = self.create_const_step_region(
            self.params.b, self.params.c, self.params.edgeStep
        )
    ...
    def create_constant_kstep_exafs(self) -> list[float]:
        ...
        k_steps = self.create_const_step_region(
            k_start, k_end, self.params.exafsStep, True
        )
    ...
    def wavevector_to_ev(self, wave_vec_angstrom: float) -> float:
        ...
        """
        return (
            XasScanParameters.wavevector_to_ev(wave_vec_angstrom)
            + self.params.edgeEnergy
        )
    ...
    def calculateVariableStepRegion(
        ...
        if de > davg:
            steps = (
                de / davg
            )  # how many average step sizes fit between A and B energies
            ...
            if num_steps >= 2:
                ...
                for i in range(0, num_steps):
                    energies.append(
                        aEnergy + preEdgeStep * i + aa * (i**2) + bb * (i**3)
                    )

                return energies
        raise Exception("Could not calculate energy points for AB region of XAS scan")

    def create_smoothed_exafs(
        self, cEnergy: float, edgeStep: float, exafsEnergies: list[float]
    ...
    def create_varying_time_exafs(
        self, energies: list[float], start_time: float, end_time: float
    ) -> list[float]:
        times = []
        const = (end_time - start_time) / math.pow(
            energies[-1] - energies[0], self.params.kWeighting
        )
        for energy in energies:
            times.append(
                start_time
                + const * math.pow(energy - energies[0], self.params.kWeighting)
            )
        return times
