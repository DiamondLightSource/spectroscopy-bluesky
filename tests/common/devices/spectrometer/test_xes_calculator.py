...
from spectroscopy_bluesky.common.devices.spectrometer.xes_spectrometer import (
    XesCalculator,
)
...
def compare_positions(
    xes_calculator: XesCalculator, filename: str, angle: float, index_range: int
):
    ...
    for index in range(-index_range, index_range + 1):
        ...
        for suffix, val in zip(suffixes, positions, strict=True):
            ...
            assert val == pytest.approx(correct_vals[dict_key]), (
                f"{dict_key} value is incorrect"
            )

    positions = xes_calculator.calculate_detector_position(angle)
    for index, dict_key in enumerate(["detX", "detY", "detRot"]):
        assert positions[index] == pytest.approx(correct_vals[dict_key]), (
            f"{dict_key} value is incorrect"
        )
...
@pytest.mark.parametrize("angle", [65.0, 70.0, 75.0, 80.0, 85.0])
